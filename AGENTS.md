...
| **Podłoga wychylenia lewej gałki przy ruchu prawą** | zmierzone przez `XInputGetState` przy `ls_min=25%`, kierunek forward: przy wychylonej prawej gałce (`R=(-1806,-2580)`, `R=(8514,3612)`) lewa pokazuje `L=(0,7998)`, czyli **31 jednostek osi na 127** — dokładnie ustawione 25 %; przy `R=(0,0)` lewa wraca do `L=(0,0)`. Znak dodatni w Y to po inwersji z §4.37 kierunek do przodu. **Funkcja fabrykuje wejście, nie przelicza go** — szczegóły i zastrzeżenie w `bridge_config.h` |
