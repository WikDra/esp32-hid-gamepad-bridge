...
esp_hidh_dev_t *esp_ble_hidh_dev_open(uint8_t *bda, uint8_t address_type)
{
    ...
    /*
     * LOCAL PATCH NOTE (AGENTS.md 4.35): initiating from a RANDOM address was tried here
     * and it is WRONG. Keeping the note so nobody repeats it.
     *
     * The idea was reasonable: esp_hidh hardcodes the public address, Windows dials the
     * troublesome keyboard from a private address and succeeds, and the initiator's own
     * address type was the last untested difference. Implemented, it changed nothing for
     * that keyboard (still "Connection failed; status=13" at rssi -53, as good a signal as
     * the board that works) - and it BROKE reconnection of already-bonded devices.
     *
     * Why it breaks them: a bonded peer comes back with DIRECTED advertising, and ADV_DIRECT_IND
     * carries the initiator's address in the packet. A device that bonded with our public
     * address directs its advertisements at that address, so an initiator using a different
     * own address must not - and does not - answer them. On hardware this showed up as a
     * keyboard advertising DIR_IND every 500 ms from a stable identity address at -42 dBm,
     * with our connection attempt timing out anyway.
     */
